...
class _SettingsScreenState extends State<SettingsScreen>
  ...
  Future<void> _updatePreferences(UserPreferences newPreferences) async {
    ...
    // Ana uygulamaya ayarların değiştiğini bildir
  ...
  Future<void> _handleBiometricToggle(bool value) async {
    if (value) {
      // Biometric özelliğini açmaya çalışıyor
      ...
      if (!isAvailable) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Bu cihazda biyometrik kimlik doğrulama desteklenmiyor'),
      ...
      // Biometric kimlik doğrulama dene
      final bool authenticated = await _biometricService.authenticate(
        reason: 'Biyometrik kimlik doğrulamayı etkinleştirmek için doğrulama yapın',
      );

      if (authenticated) {
        _updatePreferences(
          _userPreferences!.copyWith(biometricAuth: true),
        );
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Kimlik doğrulama başarısız'),
              backgroundColor: Colors.red,
            ),
          );
        }
      ...
      // Biometric özelliğini kapatıyor
      _updatePreferences(
        _userPreferences!.copyWith(biometricAuth: false),
      );
    }
  }
